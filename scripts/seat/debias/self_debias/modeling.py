...
from scripts.seat.debias.self_debias.generation import (
    ...
    SelfDebiasingLlama2LMHeadModel, # Added for llama 2 models
    SelfDebiasingPhi2LMHeadModel, # Added for Phi 2 models
)
...
class GPT2Wrapper(GenerativeLMWrapper):
    def __init__(self, model_name: str = "gpt2-xl", use_cuda: bool = True):
        ...
        self._model = SelfDebiasingGPT2LMHeadModel.from_pretrained(
            model_name
        ).bfloat16()  # type: SelfDebiasingGPT2LMHeadModel
        if use_cuda:
            # self._model.parallelize()
            pass
...
# Modified
# For phi 2 models
# Attempt to add Phi2 into self-debiasing
# Probably require 1. Query model batch 2. Generate 3. Generate self-debiasing 4. Compute loss 5. Compute loss self-debiasing
class Phi2Wrapper(GenerativeLMWrapper):
    def __init__(self, model_name: str = "microsoft/phi-2", use_cuda: bool = True):
        super().__init__(use_cuda=use_cuda)
        self._tokenizer = AutoTokenizer.from_pretrained(model_name)
        self._model = SelfDebiasingPhi2LMHeadModel.from_pretrained(
            model_name
        ).bfloat16()  # type: SelfDebiasingPhi2LMHeadModel
        if use_cuda:
            # self._model.parallelize()
            pass
        self._model.to("cuda" if torch.cuda.is_available() else "cpu")
...
# Probably require 1. Query model batch 2. Generate 3. Generate self-debiasing 4. Compute loss 5. Compute loss self-debiasing
class Llama2Wrapper(GenerativeLMWrapper):
    def __init__(self, model_name: str = "meta-llama/Llama-2-7b-hf", use_cuda: bool = True):
        """
        :param model_name: the name of the pretrained GPT2 model (default: "gpt2-xl")
        :param use_cuda: whether to use CUDA
        """
        ...
        self._model = SelfDebiasingLlama2LMHeadModel.from_pretrained(
            model_name
        ).bfloat16()  # type: SelfDebiasingLlama2LMHeadModel
        if use_cuda:
            # self._model.parallelize()
            pass
        self._model.to("cuda" if torch.cuda.is_available() else "cpu")
