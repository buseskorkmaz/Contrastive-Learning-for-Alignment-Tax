...
def init_debiased_model(model_name):
    ...
    if model_name == "gpt2":
        ...
        bias_direction = torch.load(f'your-path/factual-bias-mitigation/src/debiasing_algorithms/sentence_debiasing/subspaces/subspace_m-GPT2Model_c-gpt2_t-{mode}.pt')
        model = SentenceDebiasGPT2LMHeadModel('gpt2', bias_direction).to(device)
        tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
        tokenizer.pad_token_id = tokenizer.eos_token_id
    elif "inlp" in model_name:
        mode = model_name.split('-')[1] 
        projection_matrix = torch.load(f'your-path/factual-bias-mitigation/src/debiasing_algorithms/inlp/projection_matrix/projection_m-GPT2Model_c-gpt2_t-{mode}_s-0.pt')
